...
CREATE TABLE `lectores_obras` (
  `lector_id` int,
  `obra_id` int,
  `score` int,
  PRIMARY KEY (`lector_id`, `obra_id`)
);
...
ALTER TABLE `lectores_obras` ADD FOREIGN KEY (`obra_id`) REFERENCES `lectores` (`lector_id`);

ALTER TABLE `lectores_obras` ADD FOREIGN KEY (`obra_id`) REFERENCES `writing` (`write_id`);
